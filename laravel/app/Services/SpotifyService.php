...
/**
 * Spotify Web API Service for Laravel
...
class SpotifyService
{
    ...
    public function __construct()
    {
        ...
        $this->apiBaseUrl = config('services.spotify.api.base_url');
    ...
    {
        return Cache::remember('spotify_access_token', now()->addMinutes(50), function () {
            $http = Http::asForm()
            ...
            if (isset($data['error'])) {
                throw new Exception('Spotify API error: ' . ($data['error_description'] ?? $data['error']));
            }
    ...
    private function makeRequest(string $endpoint): array
    {
        ...
        if (!$response->successful()) {
            ...
            throw new Exception('Spotify API error: ' . $error . ' (HTTP ' . $response->status() . ')');
        }
    ...
    public function searchWithPreview(string $songName, string $artistName = '', int $limit = 20): array
    {
        ...
        try {
            ...
            logger()->error("Default preview search failed: " . $e->getMessage());
        }
        ...
        if (count($allResults) < $limit) {
            ...
            foreach ($priorityMarkets as $market) {
                ...
                try {
                    ...
                    logger()->error("Preview search failed for market {$market}: " . $e->getMessage());
                    continue;
                }
            }
        }
        ...
        if (empty($allResults) && !empty($artistName)) {
            try {
                ...
                logger()->error("Fallback preview search failed: " . $e->getMessage());
            }
        }
    ...
    public function getGenreFromTrack(array $track): ?string
    {
        try {
            ...
            logger()->error("Genre fetch error: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function testConnection(): bool
    {
        try {
            ...
            logger()->error("Spotify test connection failed: " . $e->getMessage());
            return false;
        }
    }
}
